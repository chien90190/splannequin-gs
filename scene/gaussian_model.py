...
class GaussianModel:
    ...
    def __init__(self, sh_degree : int, args, optimizer_type="default"):
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, cam_infos : int, spatial_lr_scale : float, time_line: int):
    ...
    def training_setup(self, training_args):
        ...
        if self.optimizer_type == "default":
            ...
            try:
                ...
                print(f"The rasterizer is not the special version. Using normal Adam.")
                self.optimizer = torch.optim.Adam(l, lr=0.0, eps=1e-15)
    ...
    def densification_postfix(self, new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation, new_tmp_radii, new_deformation_table):
        d = {"xyz": new_xyz,
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        new_tmp_radii = None # self.tmp_radii[selected_pts_mask].repeat(N)
        new_deformation_table = self._deformation_table[selected_pts_mask].repeat(N)
        self.densification_postfix(new_xyz, new_features_dc, new_features_rest, new_opacity, new_scaling, new_rotation, new_tmp_radii, new_deformation_table)
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent, density_threshold=20, displacement_scale=20, model_path=None, iteration=None, stage=None):
        ...
        new_tmp_radii = None # self.tmp_radii[selected_pts_mask]
        new_deformation_table = self._deformation_table[selected_pts_mask]
        self.densification_postfix(new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation, new_tmp_radii, new_deformation_table)
    ...
    def densify(self, max_grad, min_opacity, extent, max_screen_size, density_threshold, displacement_scale, model_path=None, 
                iteration=None, stage=None, color_grad_iter=0, color_grad_weight=1):
        
        grads = self.xyz_gradient_accum / self.denom
        grads[grads.isnan()] = 0.0

        if iteration < color_grad_iter:
            color_grads = self.color_gradient_accum / self.denom
            color_grads[color_grads.isnan()] = 0.0
            combined_grads = color_grad_weight * color_grads + grads
        else:
            combined_grads = grads

        self.densify_and_clone(combined_grads, max_grad, extent, density_threshold, displacement_scale, model_path, iteration, stage)
    ...
    def add_densification_stats(self, viewspace_point_tensor, update_filter, color_point_tensor):
        self.xyz_gradient_accum[update_filter] += torch.norm(viewspace_point_tensor[update_filter,:2], dim=-1, keepdim=True)
        self.color_gradient_accum[update_filter] += torch.norm(color_point_tensor[update_filter,:, :], dim=(1,2)).unsqueeze(1)
    ...
    def save_ply_filtered(self, path, mask):
        """Save only the gaussians selected by the boolean mask"""
        mkdir_p(os.path.dirname(path))

        og_shape = self._xyz.shape[0]
        # print(mask.sum()/ mask.numel(), self._xyz.shape[0])

        # Apply mask to filter all parameters
        ...
        print(f"Saved {xyz.shape[0]} filtered gaussians of {og_shape} to {path}")

    
    def save_ply_deformed(self, path, mask, time_tensor):
        """Save deformed gaussians at specific time"""
        mkdir_p(os.path.dirname(path))
        
        # Apply mask first
        canonical_xyz = self._xyz[mask]
        canonical_scales = self._scaling[mask]
        canonical_rotations = self._rotation[mask]
        canonical_opacity = self._opacity[mask]
        canonical_shs = self.get_features[mask]
        
        # Create time tensor for selected points
        time_for_points = time_tensor.repeat(canonical_xyz.shape[0], 1)
        
        # Apply deformation network
        deformed_xyz, deformed_scales, deformed_rotations, deformed_opacity, deformed_shs = self._deformation(
            canonical_xyz, canonical_scales, canonical_rotations, canonical_opacity, canonical_shs, time_for_points
        )
        
        # Convert to numpy for saving
        xyz = deformed_xyz.detach().cpu().numpy()
        normals = np.zeros_like(xyz)
        f_dc = deformed_shs[:, :, 0:1].detach().transpose(1, 2).flatten(start_dim=1).contiguous().cpu().numpy()
        f_rest = deformed_shs[:, :, 1:].detach().transpose(1, 2).flatten(start_dim=1).contiguous().cpu().numpy()
        opacities = deformed_opacity.detach().cpu().numpy()
        scale = deformed_scales.detach().cpu().numpy()
        ...
        print(f"Saved {xyz.shape[0]} deformed gaussians to {path}")
